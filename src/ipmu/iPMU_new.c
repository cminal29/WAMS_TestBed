...
unsigned char *lSecret;
 ...
 struct timeval ste,ete;
    ...
    printf("%d \n length of lsecret is \n",lSecretLen);
   
    if (!RAND_bytes(iv, sizeof(iv))) {
        //fprintf(stderr, "ERROR: RAND_bytes error: %s\n", strerror(errno));
         die("Failed to generate random IV initialization vector.\n");

        //return errno;
     }

     cipher_type=EVP_aes_256_cbc();
     cipher_block_size = EVP_CIPHER_block_size(cipher_type);
	...
	//unsigned int ll_plain=0;
        //while(*(cfg2_frm+ll_plain)) ll_plain++;
printf("\n Before encryption:-");
BIO_dump_fp (stdout, (const char *)cfg2_frm, cfg2_frm_size);

	printf("string length is:-%d",strlen ((char *)cfg2_frm));
        unsigned char ciphertext[cfg2_frm_size];

	static const unsigned char key[]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
...
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	...
	BIO_dump_fp (stdout, (const char *)enc_out, 16);
	while(1);

	ciphertext_len = AES_encrypt (cfg2_frm, enc_cfg2_frm_size, lSecret , iv,
                            ciphertext);
	strncpy(cfg2_frm,ciphertext,ciphertext_len);

	gettimeofday(&ete,NULL);
	printf("\n Cipher text length is:-%d",ciphertext_len);
...
/*			B_copy(cfg2_frm,cfg_info->cfg_digital_dgunit,indx,4*cfg_info->cfg_dgnmr_val);
	...
	gettimeofday(&ste,NULL);
	 ciphertext_len = encrypt (cfg2_frm, strlen ((char *)cfg2_frm), lSecret , iv,
                            ciphertext);
	  gettimeofday(&ete,NULL);
	printf("\n Cipher text length is:-%d",ciphertext_len);
 	int elapsed_e=((ete.tv_sec - ste.tv_sec)*1000000)+(ete.tv_usec-ste.tv_usec);
...
void generate_data_frame()
{
 ...
 struct timeval ste,ete;
	...
	unsigned int ll_plain=0;
        while(*(data_frm+ll_plain)) ll_plain++;

        unsigned char *data_frm_char_array=(unsigned char*)malloc(sizeof(data_frm));
        memcpy(data_frm_char_array,(const unsigned char *)&data_frm,sizeof(data_frm));


	gettimeofday(&ste,NULL);
	 ciphertext_len = encrypt (data_frm_char_array, strlen ((char *)data_frm_char_array), lSecret , iv,
                            ciphertext);
	  gettimeofday(&ete,NULL);
	printf("\n Cipher text length is:-%d",ciphertext_len);
 	int elapsed_e=((ete.tv_sec - ste.tv_sec)*1000000)+(ete.tv_usec-ste.tv_usec);
...
gettimeofday(&ste,NULL);
  ciphertext_len = encrypt (data_frm, strlen ((char *)data_frm), lSecret , iv,
                            ciphertext);
  gettimeofday(&ete,NULL);
 int elapsed_e=((ete.tv_sec - ste.tv_sec)*1000000)+(ete.tv_usec-ste.tv_usec);
  printf("\n \n Time for encryption %d in miroseconds\n ",elapsed_e);
