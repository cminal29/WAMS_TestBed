...
//	printf("Inside assign_df id = %d index %d front = %d,rear = %d ,soc = %d, fsec = %d\n",IDcode,index,front,rear,df_soc,df_fracsec);
	...
	if(TSB[index].soc == NULL) { // 1 if
		...
		//writeTimeToLog(30,IDcode,df_soc,df_fracsec); // Checkpoint time before sorting
...
//					printf("dd Index %d front %d  rear %d, Tcount %d ,%u,%u,%u\n",k,front,rear,Tsb_Count,IDcode,df_soc,df_fracsec);
		...
		//	writeTimeToLog(31,IDcode,tsb_soc,tsb_fracsec); // Checkpoint time before sorting
			unsigned int df_soc = to_long_int_convertor(df->soc);
//			unsigned char *fsec;
//			fsec = malloc(3*sizeof(unsigned char));
			fsec[0] = df->fracsec[1];
...
//		writeTimeToLog(3,IDcode,tsb_soc,tsb_fracsec); // Checkpoint time before sorting
	...
	// Call the applications
	//frequencyInstability(index);
	
//	writeTimeToLog(7,IDcode,tsb_soc,tsb_fracsec); // Checkpoint time after sorting
...
//			writeTimeToLog(99,IDcode,tsb_soc,tsb_fracsec); // Checkpoint time before sorting
			free(tsb_fsec);
