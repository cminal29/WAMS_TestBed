...
void* connect_pmu_udp(void *temp) {
	...
	//here we have to add decryption not sure have to ask	
	...
	unsigned char iv[AES_BLOCK_SIZE];
		...
		printf("\n  IV initialization is executed successfully");     		
     		gettimeofday(&std,NULL);
  		
		decryptedtext=decrypt_new(pSecret,iv,udp_BUF,bytes_read);
    		gettimeofday(&etd,NULL);
		int elapsed_d=((etd.tv_sec - std.tv_sec)*1000000)+(etd.tv_usec-std.tv_usec);
  		printf("\n \nTime for decryption %d in miroseconds \n",elapsed_d);
		BIO_dump_fp (stdout, (const char *)decryptedtext, bytes_read);
