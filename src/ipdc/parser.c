...
int dataparser(unsigned char data[]) { 
	...
	//following variables are added to write to file log
	unsigned int df_soc,df_fracsec,IDcode;
...
//    printf("ID = %d\n",id);
	...
	if(match){
		...
		if(!df->fracsec) {
		 ...
		 //here writing the log will let us know that at what time time 
                //timestamp value is received at PDC
                //call timelog function here
		df_soc=to_long_int_convertor(df->soc);
		df_fracsec=to_long_int_convertor(df->fracsec);
		IDcode=to_intconvertor(df->idcode);
                //writeTimeToLog(101,IDcode,df_soc,df_fracsec);
...
void writeTimeToLog(int index,unsigned int IDcode,unsigned long int l_soc,unsigned long int l_fracsec) { 
	...
	pthread_mutex_lock(&mutex_log_write);
	struct timeval tm;
	...
	if(MAX_STRING_SIZE - strlen(logTimeBuff) < 50) {

        strncpy(logTimeBuffBakup,logTimeBuff,MAX_STRING_SIZE);
        memset(logTimeBuff,'\0',MAX_STRING_SIZE);        
        strncpy(logTimeBuff,smallBuff,strlen(smallBuff));
        ...
        strcat(logTimeBuff,smallBuff);       
    }
	pthread_mutex_unlock(&mutex_log_write);
}
